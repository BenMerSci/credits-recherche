...
full_amphi_sub_qc_list_name <- unique(as.character(full_amphi_sub_qc_list$binomial)) #got them all
full_amphi_sub_metaweb <- full_amphi[which(full_amphi$binomial %in% sp_L),]
full_amphi_sub_metaweb_name <- unique(as.character(full_amphi_sub_metaweb$binomial))

#rept
full_rept_sub_qc_list <- full_rept[which(full_rept$binomial %in% sp_qc_list),]
full_rept_sub_qc_list_name <- unique(as.character(full_rept_sub_qc_list$binomial)) #got them all
full_rept_sub_metaweb <- full_rept[which(full_rept$binomial %in% sp_L),]
full_rept_sub_metaweb_name <- unique(as.character(full_rept_sub_metaweb$binomial))

#mamm
full_mamm_sub_qc_list <- full_mamm[which(full_mamm$binomial %in% sp_qc_list),]
full_mamm_sub_qc_list_name <- unique(as.character(full_mamm_sub_qc_list$binomial)) #got them all
full_mamm_sub_metaweb <- full_mamm[which(full_mamm$binomial %in% sp_L),]
full_mamm_sub_metaweb_name <- unique(as.character(full_mamm_sub_metaweb$binomial))
